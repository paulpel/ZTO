...
def local_search(solution, items, max_weights):
    ...
    for i in range(len(solution)):
        for j in range(len(max_weights)):
            ...
            if (
                evaluate_solution(improved_solution, items, max_weights)[j]
                > evaluate_solution(solution, items, max_weights)[j]
            ):
...
def bee_colony(num_solutions, num_items, max_weights, num_iterations, items):
    ...
    for _ in range(num_iterations):
        ...
        fitness_values = [
            evaluate_solution(solution, items, max_weights) for solution in population
        ]
        ...
        for i in range(num_solutions):
            ...
            if (
                evaluate_solution(neighbor, items, max_weights)[0]
                > evaluate_solution(solution, items, max_weights)[0]
            ):
            ...
            if (
                ...
                or evaluate_solution(population[i], items, max_weights)[0]
                > evaluate_solution(best_solution, items, max_weights)[0]
            ):
